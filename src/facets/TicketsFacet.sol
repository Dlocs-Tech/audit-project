...
contract TicketsFacet is IERC1155 {
    ...
    bytes4 internal constant ERC1155_ACCEPTED = 0xf23a6e61; // Return value from `onERC1155Received` call if a contract
        // accepts receipt (i.e `bytes4(keccak256("onERC1155Received(address,address,uint256,uint256,bytes)"))`).
        ...
        override
    {
        ...
        require(
            _from == msg.sender || s.accounts[_from].ticketsApproved[msg.sender], "Tickets: Not approved to transfer"
        );
     ...
     * smart contract (e.g. code size > 0). If so, it MUST call the relevant `ERC1155TokenReceiver` hook(s) on `_to` and
     * act appropriately (see "Safe Transfer Rules" section of the standard).                      
     *     @param _from    Source address
     *     @param _to      Target address
    ...
    function migrateTickets(TicketOwner[] calldata _ticketOwners) external {
        for (uint256 i; i < _ticketOwners.length; i++) {
            ...
            require(
                ticketOwner.ids.length == ticketOwner.values.length, "TicketFacet: ids and values not the same length"
            );
            // gas optimization
            unchecked {
